"""
MCP Server for arXiv article retrieval and processing.

This server provides tools to search, retrieve, and process articles from arXiv.org
...
DOWNLOAD_PATH = os.getenv("DOWNLOAD_PATH", os.path.join(os.getcwd(), "data", "arxiv"))
...
async def download_article(title: str) -> str:
    """
    Download the article hosted on arXiv.org as a PDF file. This tool can search for the article based on its
    title or accept an arXiv URL directly. It retrieves the article's PDF and saves it to a specified download
    location using the arXiv ID as the filename.

    Args:
        title: Article title or arXiv URL (e.g., "https://arxiv.org/abs/1234.5678" or "https://arxiv.org/pdf/1234.5678").

    Returns:
        Success or error message.
    """
    # Check if input is an arXiv URL
    if "arxiv.org" in title.lower():
        arxiv_id = extract_arxiv_id_from_url(title)
        if arxiv_id is None:
            return "Invalid arXiv URL format. Expected format: https://arxiv.org/abs/1234.5678 or https://arxiv.org/pdf/1234.5678"
        article_url = f"https://arxiv.org/pdf/{arxiv_id}"
    else:
        # Treat as title and search for the article
        result = await get_url_and_arxiv_id(title)
        if isinstance(result, str):
            return result
    ...
    try:
        with open(file_path, "wb") as file:
            file.write(pdf_doc)
        return f"Download successful. Find the PDF at {file_path}"
    except Exception:
